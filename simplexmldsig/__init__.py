...
PTN_KEY_INFO_RSA_KEY = \
    '<KeyInfo><X509Data><X509Certificate>%(x509)s</X509Certificate></X509Data><KeyValue><RSAKeyValue><Modulus>%(modulus)s</Modulus><Exponent>%(exponent)s</Exponent></RSAKeyValue></KeyValue></KeyInfo>'
...
def c14n(xml):
    ...
    tree.write_c14n(output, exclusive=1, with_comments=False, compression=0)
    output.flush()
...
def _generate_key_info_xml_rsa(modulus, exponent, cert):
    '''
